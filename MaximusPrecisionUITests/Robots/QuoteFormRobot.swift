...
final class QuoteFormRobot: Robot {
    ...
    /// The form keeps the keyboard up after typing; tap the header title (a
    /// non-interactive area wired to dismiss) to get it out of the way before
    /// reaching for buttons near the bottom of the screen.
    @discardableResult
    func dismissKeyboardIfNeeded() -> Self {
        guard app.keyboards.element.exists else { return self }
        app.staticTexts["Nueva Cotización"].firstMatch.tap()
        return self
    }
